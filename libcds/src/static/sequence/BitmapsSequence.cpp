...
BitmapsSequence::BitmapsSequence(uint *seq, size_t n, Mapper *am, BitSequenceBuilder *bsb, bool keepsSequence)
        : Sequence(n) {
    bsb->use();
    sigma = 0;
...
BitmapsSequence::BitmapsSequence(const Array &a, Mapper *am, BitSequenceBuilder *bsb, bool keepsSequence)
        : Sequence(0) {
    bsb->use();
    length = a.getLength();
    this->keepsSequence = keepsSequence;
