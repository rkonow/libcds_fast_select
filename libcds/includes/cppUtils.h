...
template<typename T>
void saveValue(ofstream &out, const T val) {
    assert(out.good());
...
template<typename T>
void saveValue(ofstream &out, const T *val, const size_t len) {
    assert(out.good());
    out.write((char *) val, len * sizeof(T));
}

template<typename T>
T *loadValue(ifstream &in, const size_t len) {
    assert(in.good());
    T *ret = new T[len];
...
template<typename T>
T *loadValue(const char *name, size_t &slen) {
    ifstream in(name);
    assert(in.good());
